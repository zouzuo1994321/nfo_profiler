...
| [v1.4.0](#) | 2609150002 | 2026-09-15 | 推荐+性能 | ④作品推荐第二子页签**「相似作品（智能推荐）」改名为「🎯 智能推荐」**；智能推荐新增**关键词输入**（偏向 演员/标签/番号/标题，单行回车或「再推荐一批」即生效）；智能推荐 **18 部构成要求（软性补充）**：在个性化基础流之上尽量覆盖 近半年≥2、近1月≥2、点赞作品演员共享≥1（不强制置顶、不抢占前排、避让最近已推，正常轮换不受影响，不足则尽力填充）；**数据源与扫描效率优化**：连接级 PRAGMA 调优（64MB 页缓存/256MB mmap/临时表内存化）+ 子表整批一次性写入 + 小库多进程回退阈值 50→200；**性能基准测试** `tools/bench_v140.py` → `output/benchmark_v140.md`（进程数/每批文件数/库总量 推荐值）。`tools/smoke_v140.py` 离屏冒烟全过 |
| [v1.3.9](#) | 2609140002 | 2026-09-14 | 交互+性能 | ④作品推荐交互统一为「**单击高亮 / 双击播放**」：**基于选中作品推荐**面板也借鉴智能推荐（单击仅高亮、双击播放）；**相似作品（智能推荐）扩到 3 行 18 部**（原 2 行 12 部，仍可上下滚动）；**打开/响应提速**：缩略图改为线程池**异步加载 + 缓存**（不再每次缩放重读盘/重查找，根治切到 ④「未响应」），推荐计算**延迟到首次进入 ④ 才触发**（启动不卡）。`tools/smoke_v139.py` 离屏冒烟 10 项全过 |
| [v1.3.8](#) | 2609140001 | 2026-09-14 | 推荐体系 | ④作品推荐**拆成「🎲 随机推荐 / 🎯 相似作品（智能推荐）」双独立模块**（QTabWidget）：随机模块单击选中并刷新下方「基于选中作品推荐」12 部、双击播放；智能推荐模块**单击仅高亮、绝不更新列表**、双击播放、单独「🔄 再推荐一批」换一批。新增 `smart_picks`（探索位 `explore=3` / 多样性 `diversity=0.5`，与随机 `explore=1`/`0.35` 互补，更多元）；交互语义按模块严格区分。`tools/smoke_v138.py` 离屏冒烟 8 项全过 |
| [v1.3.7](#) | 2609120005 | 2026-09-12 | 推荐质量 | ④作品推荐**随机推荐质量再优化**：**①** 近半年（180 天）作品加权（解析 `premiered`/`dateadded`，随档内 `jitter` 自适应缩放，权重 0.7，避免被偏好分 86~385 淹没）；**②** 多次浏览未投降权（拉 `play_history.play_count`，未投票且 `play_count>3` 每多 1 次扣 0.12·jitter，封顶 1.2）；**③** 👎 硬排除（绝不进候选池）。实测近半年占比 3.8%→19.0%、多次浏览未投票 13→5、👎 出现 0；回归 v1.3.4 的 A/B/C/D 无破坏 |
